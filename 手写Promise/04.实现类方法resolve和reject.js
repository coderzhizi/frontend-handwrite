...
class MyPromise {
  ...
  _runOneHandler({ executor, state, resolve, reject}) { // handler
    runMicroTask(() => {
      ...
      try {
        ...
        if (isPromise(result)) {
          // 这个this是当前then返回的promise
          // 这些个resolve是调用then中的回调后就会创建的promise（then的返回），然后回调中返回的promise
          // 由于这个then中回调有返回一个promise，then返回的promise结果又需要重新其决定
          // 因此我将当前then方法返回的promise传递给回调返回promise中，然后其中这下面then(resolve, reject)中调用resolve时调用的resolve就是then返回的promise的resolve
   ...
   * 传递的data是PromiseLike（实现了Promise A+规范），是一个有then方法的对象
   * @param {*} data 
   */
  static resolve(data) {
    if (data instanceof MyPromise) { // 如果是面试时要单独完成一个resolve方法，则写 Promise即可，下面同理
      return data
    }
